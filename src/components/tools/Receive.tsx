...
const Receive = () => {
  const API_URL = `${import.meta.env.VITE_SOME_BASE_URL}/receive`; // Update the .env file with the actual API endpoint
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setError(data.message || 'An error occurred during the receive process');
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="text-center mb-8">
          ...
          <p className="text-xl md:text-2xl text-cyan-200/80">
            Acquire site information from your guardian
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="deviceId" className="block text-cyan-200 mb-2 font-medium">
                  Crow ID (scan the QR Code with the Crow image)
                </label>
                <input
                  type="text"
                  id="deviceId"
